...
/// when the popup loads, it gets the current tab url
/// and inserts the qr code into the #qr_container div
...
copy_button.addEventListener("click", async (_e) => {
    ...
    try {
        ...
        error_container.innerText = "Unable to copy image to clipboard\n Please be sure to enable\ndom.events.asyncClipboard.clipboardItem\nin about:config";
    }
});

function slugify(text) {
    return text.replace(/\W+/g, "-").toLowerCase().replace(/-$/, "").replace(/^-/, "");
}
...
download_button.addEventListener("click", async (_e) => {
    ...
    a.href = img;
    a.download = title ? slugify(title) + "-qr-code.png"
